...
async function request<T>(
  ...
  // Redirect 307 — laisser le navigateur suivre (download MinIO)
  if (res.redirected) {
    return res.url as T
  }
...
// ─── Endpoints typés ─────────────────────────────────────────────────────────
// On importe les types générés depuis openapi.json.
// Si openapi-typescript n'a pas encore été lancé, ces imports échoueront —
// lancer : pnpm exec openapi-typescript src/api/openapi.json -o src/api/types/api.ts
...
export const sessionsApi = {
  ...
  create: (body: SessionCreate) => api.post<SessionRead>('/sessions', body),
  startAcquisition: (id: string) =>
    api.post<SessionRead>(`/sessions/${id}/start-acquisition`),
  startProcessing: (id: string) =>
    api.post<SessionRead>(`/sessions/${id}/start-processing`),
...
export const assetsApi = {
  ...
  // Retourne l'URL présignée MinIO (après redirect 307)
